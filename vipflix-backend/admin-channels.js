...
function readCustomChannels(customChannelsPath) {
  if (!fs.existsSync(customChannelsPath)) return [];
  try {
    const data = JSON.parse(fs.readFileSync(customChannelsPath, 'utf8'));
...
function writeCustomChannels(customChannelsPath, events) {
  const dir = path.dirname(customChannelsPath);
  if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
  fs.writeFileSync(customChannelsPath, JSON.stringify(events, null, 2));
}

function renderAdminChannelsPage() {
  ...
  const clientJs = [
    ...
    '',
    'function render() {',
    "  var root = document.getElementById('channels');",
    '  if (!channels.length) {',
    "    root.innerHTML = '<div class=\"empty\">No channels yet. Click <strong>Add Channel</strong>.</div>';",
    '    return;',
    '  }',
    '',
    '  var html = "";',
    ...
    '    html += \'<div class="card-title">#\' + (i + 1) + \' · \' + esc(ev.title || "Untitled channel") + \'</div>\';',
    ...
    '    html += \'<button class="reorder" type="button" data-move="down" data-index="\' + i + \'"\' + (i === channels.length - 1 ? " disabled" : "") + \' title="Move down">Down</button>\';',
    ...
    '    html += \'<div class="muted">Order: \' + (i + 1) + \' of \' + channels.length + \'</div></div></div>\';',
    '  }',
    '  root.innerHTML = html;',
    '',
    ...
    '      syncFromDom();',
    '      channels.splice(Number(btn.getAttribute("data-remove")), 1);',
    '      render();',
    '    });',
    '  });',
    '',
    ...
    '      syncFromDom();',
    '      var tmp = channels[index];',
    '      channels[index] = channels[target];',
    '      channels[target] = tmp;',
    '      render();',
    ...
    '    var card = cards[i];',
    '    var next = Object.assign({}, channels[i] || blankChannel());',
    ...
    '    }',
    '    nextChannels.push(next);',
    '  }',
    '  channels = nextChannels;',
    '}',
    '',
    'function loadEvents() {',
    "  return fetch('/api/custom-channels')",
    '    .then(function (res) { return res.json(); })',
    '    .then(function (data) {',
    '      channels = Array.isArray(data) ? data : [];',
    '      render();',
    '      setStatus("Loaded " + channels.length + " channel(s).", true);',
    '    });',
    '}',
    '',
    'function saveEvents() {',
    ...
    '    setStatus("Saved " + channels.length + " channel(s). App will pick these up on next Home refresh.", true);',
    '  });',
    '}',
    '',
    "document.getElementById('addBtn').addEventListener('click', function () {",
    '  syncFromDom();',
    '  channels.push(blankChannel());',
    '  render();',
    '});',
  ...
  return [
    ...
    '  <title>VIPFLIX Exclusive Channels</title>',
    '  <style>',
    css,
    '  </style>',
    '</head>',
    '<body>',
    '  <div class="wrap">',
    '    <h1>VIPFLIX Exclusive Channels</h1>',
    '    <p class="sub">These show on the app <strong>Home</strong> page in a row called <strong>Exclusive Channels</strong>. Saved to <code>data/custom-channels.json</code>.</p>',
    '    <div class="help">',
    '      <strong>Channel ID</strong> = your Xtream <strong>stream ID</strong> (numeric <code>streams.id</code> from XUI).',
    '      When a user taps the item, the app plays that channel. Leave blank only if you want title-only promos.',
    ...
    '      Confirm AM/PM on each channel, then click <strong>Save All</strong>.',
    '    </div>',
    '    <div class="toolbar">',
    '      <button class="primary" id="addBtn" type="button">+ Add Channel</button>',
...
module.exports = {
  readCustomChannels: readCustomChannels,
  writeCustomChannels: writeCustomChannels,
  renderAdminChannelsPage: renderAdminChannelsPage
};
